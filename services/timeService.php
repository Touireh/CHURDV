...
class timeService implements IDao {
    ...
    public function findAlz($secretaire,$jour) {
        $query = "select * from temps where secretaire =? and jour=DAYOFWEEK(?) and CONCAT(id,?) NOT IN (select verify from reference)";
        $req = $this->connexion->getConnexion()->prepare($query);
        $req->execute(array($secretaire,$jour,$jour));
        $f= $req->fetchAll(PDO::FETCH_OBJ);
        return $f;
    }
    ...
    public function findById($id) {
        ...
        //$fonction = new Time($res->id, $res->temps, $res->secretaire, $res->jour);
        return $res;
    }
    ...
    public function findId($time,$date) {
        ...
        $res = $req->fetch(PDO::FETCH_OBJ);
        $hor = new Time($res->id, $res->temps, $res->secretaire, $res->jour);
        return $hor;
    }
